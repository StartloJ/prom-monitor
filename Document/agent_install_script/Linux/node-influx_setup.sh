...
mkdir /etc/influxdb_exporter
cp ./node_exporter*/* /etc/node_exporter/
cp ./influxdb_exporter*/* /etc/influxdb_exporter/
...
chown prometheus:prometheus /etc/influxdb_exporter/*
chmod +x /etc/node_exporter/*
chmod +x /etc/influxdb_exporter/*
...
cat <<EOF | tee /etc/systemd/system/influxdb_exporter.service
#### Influxdb_exporter for OS metrics ####
...
[Service]
User=prometheus
ExecStart=/etc/influxdb_exporter/influxdb_exporter
Restart=on-failure
...
systemctl status influxdb_exporter